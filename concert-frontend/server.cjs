...
const LIBRARY_ROOT = path.join(ROOT, 'public', 'music');
const VITE_PORT = 5173;
const AUDIO_EXTS = new Set(['.mp3', '.m4a', '.aac', '.ogg', '.wav', '.mgg']);
const IMAGE_EXTS = new Set(['.jpg', '.jpeg', '.png', '.webp', '.gif']);

function toUrl(absPath) {
  return '/music/' + path.relative(LIBRARY_ROOT, absPath).split(path.sep).join('/');
}
...
function serve(req, res) {
  ...
  if (pathname.startsWith('/music/')) {
    const abs = safeJoin(LIBRARY_ROOT, pathname.replace('/music/', '/'));
